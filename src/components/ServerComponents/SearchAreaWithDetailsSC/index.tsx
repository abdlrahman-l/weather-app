...
export default async function SearchAreaWithDetailsSC({
  ...
  const area =
    formattedData?.find?.((d) => d.id === areaId) || formattedData[0];
  const { paramObj } = area;
  ...
  const groupedTimerange = paramObj.weather?.timerange.reduce<{
    ...
    // hourly
    const timeIndex = paramObj.temperature?.timerange.findIndex(
      (t) => t.datetime === curr.datetime
    );
  ...
  return (
    <div>
      <SearchArea areas={formattedData} defaultArea={area} />
